...
# shared, for 2019-2020
dlmi <- read.csv("NC_DLMI.csv")
dlmi <- dlmi %>% 
  rename(region = SBE.Region, psu = PSUCODE, number = SCHOOL.CODE, name = SCHOOL.NAME, 
         access = X2019.20...of.Students.with.Home.Internet.Access)
dlmi$access <- as.integer(dlmi$access)
dlmi$region[dlmi$region == ""] <- NA
dlmi$region[dlmi$region == "#N/A"] <- NA
```
...
dlmi %>% 
  group_by(region) %>% 
  summarize(median = median(access, na.rm = TRUE))
```
...
ggplot(data = dlmi, mapping = aes(x = region, y = access)) +
  geom_point(alpha = 0.25)
```