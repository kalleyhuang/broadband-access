...
home_internet <- home_internet %>% 
  rename(district = LEA.Name, number = School.Number, name = School.Name1, 
...
dlmi <- dlmi %>% 
  rename(region = SBE.Region, district = PSUCODE, number = SCHOOL.CODE, name = SCHOOL.NAME, 
         access = X2019.20...of.Students.with.Home.Internet.Access)
dlmi$district <- as.integer(dlmi$district)
dlmi$number <- as.integer(dlmi$number)
